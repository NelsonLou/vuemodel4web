...
<div class="searchForm">
	<Form :model="filterForm" ref="filterForm" inline v-if='showForm'>
		<FormItem :prop="item.valName" v-for='(item,index) in configList' :key='index'>
			...
			<!-- 选择器 -->
			<Select v-if='item.type=="select"' @onchange="handleSearch" :style="{'width':item.width?item.width+'px':'150px'}" type="text" v-model="filterForm[item.valName]" :placeholder="item.plh" clearable class="filterInput">
			...
			<DatePicker v-if='item.type=="dateRange"' @onchange="handleSearch" :placeholder="item.plh" v-model="dateRange" type="datetimerange" class="DatePicker"></DatePicker>
			<!-- 地区级联选择器 -->
			<Cascader v-if='item.type=="region"' @onchange="handleSearch" style="width:170px" v-model='filterForm[item.valName]' :data="region" :placeholder="item.plh" change-on-select clearable class="filterInput"></Cascader>
		</FormItem>
